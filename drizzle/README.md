...
`npm run db:migrate` applies `src/server/db/migrations/*.sql` in lexical order.
...
2. `npm run db:generate` — writes a diff here.
...
5. `npm run db:migrate`, then `npx vitest run schema-drift rls` to prove the
