...
| `undici` | HTTP client for every outbound request the server makes to a customer-configured destination | The only available way to pin a connection to an address we already validated. `Agent({ connect: { lookup } })` decides where the socket goes, which is what closes the DNS-rebinding window in `egress-guard.ts`; Node's built-in `fetch` exposes no equivalent and lets no dispatcher of ours be constructed for it | `node:https` with a custom `lookup` (rejected: would mean hand-writing redirect, streaming, header and abort handling that `fetch` already gets right); a forward proxy (deferred: infrastructure rather than code, and it moves the decision out of the application) | Adopted at `^7.29.1`. Zero transitive dependencies. **Deliberately not 8.x**, for two independent reasons: undici 8 declares `engines.node >= 22.19.0` and this project runs 22.17.0, and its dispatcher interface is incompatible with Node's built-in `fetch` (`invalid onRequestStart method`). jsdom already carried undici 8 as a dev-only transitive dependency; that copy is not what production uses, and depending on it was not an option |
