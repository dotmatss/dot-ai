...
- **Firebase is the identity provider where it is configured** (ADR 0007). The browser authenticates to Firebase, so passwords never reach this server at all; the ID token is verified against Google's published JWKS - checking `alg`, `kid`, the signature, `aud`, `iss`, `exp`, `iat`, `auth_time` and `sub` - and exchanged **once** for the application session below, which is revocable by a row delete where an ID token would stay valid for its hour. Setting `FIREBASE_PROJECT_ID` also refuses the password Server Actions, because a Server Action is an HTTP endpoint and hiding the form would leave a second, weaker way in. An external identity whose address Firebase has not verified may never adopt an account that already exists - that email match would otherwise be an account-takeover primitive. Authorization is untouched: `organization_members.role` and `platform_admins` decide everything, for a Firebase token exactly as for a cookie. See `docs/firebase-auth.md`.
