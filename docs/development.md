...
npm run db:migrate                # applies src/server/db/migrations/*.sql
...
| `npm run verify` | **Preferred check.** Lock-protected `next typegen` → `tsc` → ESLint → Vitest |
| `npm run typecheck` | `tsc --noEmit` (run `npx next typegen` first if you added routes) |
| `npm run lint` | ESLint (Next.js core-web-vitals + TypeScript rules) |
| `npm test` / `npm run test:watch` | Vitest unit + component tests |
| `npm run test:e2e` | Playwright journeys (`npx playwright install chromium` once; starts the dev server if needed) |
| `npm run db:migrate` | Apply pending migrations; `-- --status` lists them |
| `npm run db:seed` | Demo user, organization, workspace and chatbot (development only) |
| `npm run db:generate` | Diff `src/server/db/schema/` and write SQL to `drizzle/` **for review** — never applies anything |
| `npm run db:check` | Validate the drizzle snapshots are consistent |

### Verifying safely

`npm run verify` exists because `next typegen` and incremental `tsc` write into shared generated directories. Two concurrent runs — a second terminal, a running dev server, or a parallel agent — interleave those writes and leave corrupt files in `.next/**` that fail as syntax errors inside generated code. The script serializes runs with an on-disk lock, disables incremental compilation, and can report only the problems you care about:

```bash
npm run verify                                                    # whole project
...
If generated types ever do get corrupted, stop the dev server, delete `.next`, and run `npm run verify` again.
...
```bash
DATABASE_URL="postgresql://postgres:<password>@localhost:5432/dot" npx vitest run chatbot-chat
```
...
2. `npm run db:generate` — writes a diff into `drizzle/`. Nothing is applied.
...
5. `npm run db:migrate`, then run the `schema-drift` and `rls` tests with `DATABASE_URL` set to prove the TypeScript, the database and the isolation policies all still agree.
