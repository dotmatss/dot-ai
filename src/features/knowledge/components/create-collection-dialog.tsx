...
import { useCreateCollectionMutation } from "@/features/knowledge/mutations";
import { createCollectionSchema, type CreateCollectionInput } from "@/features/knowledge/schemas";
import { useWorkspace } from "@/features/workspaces/components/workspace-provider";
import { canEdit } from "@/features/workspaces/roles";
import { isApiError } from "@/lib/api/api-error";
import { applyFieldErrors } from "@/lib/forms/apply-field-errors";

const FORM_ID = "create-collection-form";

type CreateMutation = ReturnType<typeof useCreateCollectionMutation>;

function CreateCollectionForm({ mutation, onCreated }: { mutation: CreateMutation; onCreated: () => void }) {
  ...
  const onSubmit = form.handleSubmit((values) => {
    mutation.mutate(values, {
      onSuccess: (base) => {
        onCreated();
        router.push(`/w/${membership.workspace.slug}/knowledge/collections/${base.id}` as Route);
      },
      onError: (error) => {
  ...
  return (
    <form id={FORM_ID} onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
      ...
      <AppFormField
        label="Description"
        optional
        description="Shown in lists and when attaching this collection to a chatbot or agent."
...
export function CreateCollectionButton({ size }: { size?: "sm" | "md" }) {
  ...
  return (
    <>
      ...
      >
        <CreateCollectionForm mutation={mutation} onCreated={() => setOpen(false)} />
      </AppDialog>
    </>
  );
}
