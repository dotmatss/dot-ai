 ...
 *
 * Every statement filters on `workspace_id = $1`, where the id comes from the
 * membership resolved by `requireWorkspaceAccess` — never from the client.
 *
...
export async function getMessagesByChatbot(workspaceId: string, period: AnalyticsPeriod): Promise<ChatbotMessagesRow[]> {
  const { days } = gridParams(period);
  const rows = await query<{ chatbot_id: string | null; name: string | null; messages: string }>(
    `SELECT c.chatbot_id::text AS chatbot_id, cb.name, count(m.id) AS messages
     FROM messages m
     JOIN conversations c ON c.id = m.conversation_id AND c.workspace_id = $1
     LEFT JOIN chatbots cb ON cb.id = c.chatbot_id AND cb.workspace_id = $1
     WHERE m.workspace_id = $1
       AND m.role IN ('user', 'assistant')
       AND m.created_at >= now() - ($2::int * interval '24 hours')
     GROUP BY c.chatbot_id, cb.name
     ORDER BY messages DESC, cb.name NULLS LAST`,
    [workspaceId, days],
  );
  return rows.map((row) => ({
...
export async function getTokenUsage(workspaceId: string, period: AnalyticsPeriod): Promise<TokenUsage> {
  ...
  const [series, byChatbot] = await Promise.all([
    ...
    query<{ chatbot_id: string; name: string | null; tokens_in: string; tokens_out: string }>(
      `SELECT u.ref_id::text AS chatbot_id, cb.name,
              coalesce(sum(u.quantity) FILTER (WHERE u.kind = 'tokens_in'), 0)  AS tokens_in,
              coalesce(sum(u.quantity) FILTER (WHERE u.kind = 'tokens_out'), 0) AS tokens_out
       FROM usage_events u
       LEFT JOIN chatbots cb ON cb.id = u.ref_id AND cb.workspace_id = $1
       WHERE u.workspace_id = $1
         AND u.ref_type = 'chatbot'
         AND u.ref_id IS NOT NULL
         AND u.kind IN ('tokens_in', 'tokens_out')
         AND u.occurred_at >= now() - ($2::int * interval '24 hours')
       GROUP BY u.ref_id, cb.name
       ORDER BY sum(u.quantity) DESC
       LIMIT $3`,
      [workspaceId, days, TOKEN_TABLE_LIMIT],
    ),
  ]);
